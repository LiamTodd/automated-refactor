...
def rename_variables_in_files(directory, old_variable_name, new_variable_name):
    ...
    def update_variable_references(node):
        if isinstance(node, ast.Name) and node.id in renamed_variables:
            node.id = renamed_variables[node.id]
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            if file.endswith(".py"):
                ...
                class RenameVariables(ast.NodeTransformer):
                    ...
                    def visit_Name(self, node):
                        if  node.id == old_variable_name:
                            ...
                            index = sum(len(line)+1 for i, line in enumerate(content.splitlines()) if i < line_number - 1) + col_offset
                ...
                modified_content = content.replace(f"import {old_variable_name}", f"import {new_variable_name}")
                modified_content = modified_content.replace(f"from {old_variable_name}", f"from {new_variable_name}")
...
if __name__ == "__main__":
    ...
    modified_files = rename_variables_in_files(directory_to_refactor, old_name, new_name)
